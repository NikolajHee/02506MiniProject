...
class TRAIN_EM(Dataset):
    ...
    def __init__(self, data_path, resize_=None, patch_size=None):
        ...
        self.train_images_path = sorted(
            glob.glob(os.path.join(data_path, "train_images", "*.png")),
        )

        self.train_labels_path = sorted(
            glob.glob(os.path.join(data_path, "train_labels", "*.png")),
        )
    ...
    def __getitem__(self, idx):
        if self.patch_size is not None:
            ...
            lbl = self.labels[idx]
            return (
                img.view(1, self.patch_size, self.patch_size) / 255,
                lbl.view(1, self.patch_size, self.patch_size) / 255,
            )
        else:
            return (
                torch.tensor(io.imread(self.train_images_path[idx])).view(1, 512, 512)
                / 255,
                torch.tensor(io.imread(self.train_labels_path[idx])).view(1, 512, 512)
                / 255,
            )
...
class TEST_EM(Dataset):
    ...
    def __init__(self, data_path, resize_=None, patch_size=None):
        ...
        self.test_images_path = glob.glob(
            os.path.join(data_path, "test_images", "*.png"),
        )
    ...
    def __getitem__(self, idx):
        if self.patch_size is not None:
            ...
            return (
                torch.tensor(io.imread(self.test_images_path[idx])).view(1, 512, 512)
                / 255
            )
